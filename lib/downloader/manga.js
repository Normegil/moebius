...
module.exports.download = function download(title, path, poolSize) {
  let emitter = new EventEmitter();
  return {
    events: emitter,
    start: function start(useCache) {
      return new Promise(function start(resolve, reject) {
        ...
        fs.mkdir(directory)
          .then(function listManga() {
            ...
            return connector.loadMangas(useCache);
            ...
            let promises = mangaInfo.chapters.map(function toPromise(chapter) {
              let downloader = chapterLib.download(chapter, directory, poolSize);
              manageEvents({
                manga: manga,
                emitter: emitter,
                step: {
...
function manageEvents(mangaOpts, events) {
  ...
  events.on(chapterEventsDef.image.downloading, function onProgress(args) {
    sendEvent(mangaOpts.emitter, eventsDefinition.chapter.image.downloading, {
      manga: mangaOpts.manga,
      step: mangaOpts.step,
      chapterEvent: args,
    });
  });
  events.on(chapterEventsDef.image.downloaded, function onProgress(args) {
    sendEvent(mangaOpts.emitter, eventsDefinition.chapter.image.downloaded, {
      manga: mangaOpts.manga,
      step: mangaOpts.step,
      chapterEvent: args,
    });
